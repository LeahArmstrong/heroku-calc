...
func GenerateMarkdown(appName string, result *config.AnalysisResult) string {
	...
	// Footer
	sb.WriteString("---\n\n")
	sb.WriteString("*Report generated by [Heroku Config Analyzer](https://github.com/leaharmstrong/heroku-calc)*\n")
...
func generateExecutiveSummary(result *config.AnalysisResult) string {
	...
	if criticalCount > 0 {
		sb.WriteString(fmt.Sprintf("🔴 **Critical Issues Found:** %d issue(s) require immediate attention  \n", criticalCount))
	}
	if warningCount > 0 {
		sb.WriteString(fmt.Sprintf("🟡 **Warnings:** %d recommendation(s) for optimization  \n", warningCount))
	}
	if criticalCount == 0 && warningCount == 0 {
		sb.WriteString("🟢 **Status:** Configuration appears optimal  \n")
	}
...
func formatRecommendation(rec config.Recommendation) string {
	...
	if rec.AutoApply {
		sb.WriteString("| **Auto-Apply** | ✅ Yes |\n")
	} else {
		sb.WriteString("| **Auto-Apply** | ❌ Manual Required |\n")
	}
	sb.WriteString("\n")
